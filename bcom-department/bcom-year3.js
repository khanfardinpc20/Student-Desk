...
const STORAGE_KEY = "bcom_year3_students_data";
...
document.getElementById("search-bar").addEventListener("input", function () {
    ...
    document.querySelectorAll("#student-table-body tr").forEach(row => {
        ...
        row.style.display = roll.includes(value) || name.includes(value) ? "" : "none";
    });
});
...
tableBody.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("edit-btn")) {
        ...
        // EDIT MODE
        for (let i = 1; i <= 9; i++) {
            cells[i].innerHTML = `<input class="edit-input" value="${cells[i].textContent.replace("₹", "")}">`;
        }
...
document.getElementById("export-btn").addEventListener("click", () => {
    ...
    a.download = "B.com_Year3_Students.csv";
    a.click();
});
